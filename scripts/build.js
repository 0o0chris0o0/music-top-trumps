...
const templateFile = process.env.PHP ? paths.templatePhp : paths.templateHtml;
if (!checkRequiredFiles([templateFile, paths.appIndexJs])) {
  process.exit(1);
}
...
async function build(previousFileSizes) {
  ...
  if (!appName || appName === 'rac-starter-kit') {
    ...
    if (process.env.NODE_ENV === 'production') {
      const err = new Error(`${chalk.red(`${appName ? 'Using template' : 'No'} project name and in production`)}
        \nmake sure to update this in package.json before redeploying`);
      throw err;
    } else {
      console.log('make sure to update this in package.json before deploying to production');
    }
    console.log('----');
  }
